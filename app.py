...
def carregar_dados_salvos():
    try:
        df = conn.read(worksheet="Página1", ttl=0) # "Página1" é o nome da aba inferior da planilha
...
if 'usuarios_db' not in st.session_state:
    st.session_state['usuarios_db'] = {'admin': 'admin123'}
...
if not st.session_state['logado']:
    st.title("Acesso ao Sistema Analítico de Exames")
    ...
    with aba_cadastro:
        ...
        if st.button("Realizar Cadastro"):
            if novo_usuario in st.session_state['usuarios_db']:
                ...
                st.success("✅ Cadastro realizado! Vá na aba 'Entrar'.")
            else:
                st.warning("⚠️ Preencha nome e senha.")

    with aba_login:
        st.subheader("Fazer Login")
        usuario = st.text_input("Seu Nome ou Login:")
...
menu = st.sidebar.radio("Navegação", ["📂 Upload de Dados", "🏢 Dashboard por Unidade", "📈 Comparativo Mensal"])
